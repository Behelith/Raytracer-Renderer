...
int Sphere::intersect(Ray& ray, float distance)
{
	...
	float3 v = ray.getOrigin() - center;									// v = o - c
	float b = -float3::dot(v, ray.getDirection());					// b = -(v dot d) -> b = -((o - c) dot d)
	float delta = (b * b) - float3::dot(v, v) + radius*radius;		// WYZNACZNIK - b2 - 4ac
	
	int returnval = MISS;
	if (delta > 0) {
		delta = sqrtf(delta);
		float i1 = b - delta;		// b - sqrt(b2-4ac)
		float i2 = b + delta;		// b + sqrt(b2-4ac)
		if (i2 > 0) {
			if (i1 < 0) {
				if (i2 < distance) { // punkt jest W srodku sfery - np gdy promien ma poczatek w tym samym punkcie co sfera
					distance = i2;
					cout << "\nOdleglosc prmienia od sfery = " << distance;
					returnval = INSIDE;
					cout << "\nPromien przecial sfere w punkcie: (" << (ray.getOrigin() + (float3(distance,distance,distance)*ray.getDirection())) << ")";
				}
			}
			else {
				if (i1 < distance) { // promien przecina sfere "z zewnatrz"
					distance = i1;
					cout << "\nOdleglosc zrodla od sfery = " << distance;
					returnval = HIT;
					cout << "\nPromien przecial sfere w punkcie : (" << (ray.getOrigin() + (float3(distance, distance, distance)*ray.getDirection())) << ")";
				}
			}
		...
		if (delta == 0) {
			...
			float i1 = b;
			distance = i1;
			cout << "\nPromien jest styczny do powierzchni sfery!";
			cout << "\nPromien przecial sfere w punkcie: (" << (ray.getOrigin() + (float3(distance, distance, distance)*ray.getDirection())) << ")";
		}

		else cout << "\nPromien nie przecial sfery!";
	}
	return returnval;
